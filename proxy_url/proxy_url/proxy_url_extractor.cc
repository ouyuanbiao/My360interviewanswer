    ...
    void ProxyURLExtractor::Extract( const KeyItems& keys, const std::string& raw_url, std::string& sub_url )
    {
        ...
        while (!token.isEnd()) {
			...
			while(token.current() != '=' && !token.isEnd()){
				if(token.current() == '&'){
					token.next();
					keystart = token.getCurReadPos();		//遇到‘&’更新键值对起始位置
				}
				else
					token.next();
			}
			...
			token.back(keyend-keystart);						//搜索到‘=’,回溯到键值对起始位置

            key = token.nextString('=');					//记录当前键值对的键
            if (keys.find(key) != keys.end()) {
                ...
                */
				...
				return;		//搜索到匹配键直接返回，不必继续搜索
            }
            if(token.skipTo('&')!='&'){      //没有搜索到匹配键，且当前已经是末尾键值对，则不再继续搜索				
				sub_url = std::string();	//返回空值
				return;
			};
            token.next();//skip one char : '&'
        }
